...
{	
	...
	/// </summary>
	[AddComponentMenu("Corgi Engine/Character/Abilities/Character Handle Melee")] 
	public class CharacterHandleMelee : CharacterAbility 
	{
		...
		public Weapon InitialMeleeWeapon;
        ...
        public Weapon UpgradedPlasmaBlade;
        ...
        public bool MeleeColliderOn = false;
        public float ReflectColliderTime = .2f;
        public float plasmaBladeAttackTime = .313f;
        protected bool airAttacking = false;
        public bool groundAttacking = false;

        public bool superPlasmaBladeObtained = false;
		...
		protected override void HandleInput ()
		{
            if (AbilityPermitted)
            {
                if ((_inputManager.DashButton.State.CurrentState == MMInput.ButtonStates.ButtonDown) || (ContinuousPress && (CurrentWeapon.TriggerMode == Weapon.TriggerModes.Auto) && (_inputManager.ShootButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)))
                {
                    MeleeStart();
                    if (_character.IsFacingRight)
                    {
                        //_controller.AddHorizontalForce (2f);
                        _controller.SetHorizontalForce(2f);
                    }
                    else
                    {
                        //_controller.AddHorizontalForce (-2f);
            ...
            /*if (_inputManager.DashButton.State.CurrentState == MMInput.ButtonStates.ButtonUp)
			{
				MeleeStop();
			}*/
		...
		/// <summary>
		/// Causes the character to start meleeing
		/// </summary>
		public virtual void MeleeStart()
		{
			// if the Melee action is enabled in the permissions, we continue, if not we do nothing.  If the player is dead we do nothing.
			if ( !AbilityPermitted
				|| (CurrentWeapon == null)
				|| (_condition.CurrentState != CharacterStates.CharacterConditions.Normal)
				|| (_movement.CurrentState == CharacterStates.MovementStates.LadderClimbing)
				|| _lucyFlamethrower.Jetpacking)
			{
				return;
			}
            ...
            StartCoroutine(MeleeColliderOnOff());

            //if airbourn, start coroutine.
            if(!_controller.State.IsGrounded)
            {
                StartCoroutine(AirAttackHappening());
            }
            //if grounded start grounded coroutine
            if (_controller.State.IsGrounded)
            {
                StartCoroutine(GroundAttackHappening());
		...
		/// <summary>
		/// Causes the character to stop meleeing
		/// </summary>
		public virtual void MeleeStop()
		{
			// if the Melee action is enabled in the permissions, we continue, if not we do nothing
			if (!AbilityPermitted
				|| (CurrentWeapon == null))
			{
				return;		
       ...
       public virtual void ChangeWeapon(Weapon newWeapon, string weaponID)
		{
			...
			{			
				...
				CurrentWeapon.SetOwnerMelee (_character, this);
        ...
        public virtual void ObtainedSuperPlasmaBlade()
        {
            superPlasmaBladeObtained = true;
            ChangeWeapon(UpgradedPlasmaBlade, null);
        }

        protected IEnumerator MeleeColliderOnOff()
        {
            MeleeColliderOn = true;
            yield return new WaitForSeconds(ReflectColliderTime);
            MeleeColliderOn = false;
        }

        protected IEnumerator AirAttackHappening()
        {
            airAttacking = true;
            yield return new WaitForSeconds(plasmaBladeAttackTime);
            airAttacking = false;
        }

        protected IEnumerator GroundAttackHappening()
        {
        ...
        protected virtual void CheckIfGroundedWhileAttacking()
        {
            //if in the air and become grounded, stop attack.
            if (airAttacking)
            {
                if(_controller.State.JustGotGrounded)
                {
                    MeleeStop();
                    StopCoroutine(AirAttackHappening());
                    airAttacking = false;
                    //give little bounce so animation doesn't get stuck.
                    _controller.SetForce(new Vector2(0f, 5f));
                }
            }
            //if grounded attacking, and somehow become airbourn, stop attack as well.
            if (groundAttacking)
            {
                if(_controller.State.JustGotGrounded)
                {
