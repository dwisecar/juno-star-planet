...
{   
    ...
    public class LucyWallClinging : CharacterWallClinging 
    {
        ...
        public bool MaglockBootsObtained = false;
        ...
        protected override void HandleInput()
        {
            ...
            //if player has the super boot upgrade, they can lock to the side of a wall with the aim lock button. 
            if(MaglockBootsObtained == true)
            {
                if (_inputManager.AimLockButton.State.CurrentState == MMInput.ButtonStates.ButtonDown)
                {
                    WallClingingSlowFactor = 0.01f;
                }
                if (_inputManager.AimLockButton.State.CurrentState == MMInput.ButtonStates.ButtonUp)
                {
                    WallClingingSlowFactor = 0.85f;
                }
            }
        ...
        protected override void WallClinging()
        {
                ...
                || ( (_controller.State.IsCollidingRight) && (_horizontalInput >= _inputManager.Threshold.x)))
            {
                ...
                //Flipping the weapon direction when wall sliding.
                if (_movement.CurrentState == CharacterStates.MovementStates.WallClinging) 
                {
                    ...
                    if ((_controller.State.IsCollidingLeft) && (_horizontalInput <= -_inputManager.Threshold.x)) 
                    {
                        ...
                        if (_characterHandleWeapon.CurrentWeapon.Flipped == true) 
                        {
                            _characterHandleWeapon.CurrentWeapon.FlipWeapon();
                        }

                    }
                
                    if ((_controller.State.IsCollidingRight) && (_horizontalInput >= _inputManager.Threshold.x)) 
                    {
                        
                        if (_characterHandleWeapon.CurrentWeapon.Flipped == false) 
                        {
                            _characterHandleWeapon.CurrentWeapon.FlipWeapon();
                        }
                    }
        ...
        protected override void ExitWallClinging()
        {
            if (_movement.CurrentState == CharacterStates.MovementStates.WallClinging)
            {
                ...
                if (shouldExit)
                {
                    StartCoroutine(LetGoOfWallSlowly());
        ...
        protected virtual IEnumerator LetGoOfWallSlowly()
        {
        ...
        /// </summary>
        protected override void WallClingingLastFrame()
        {
            ...
            //Flipping the weapon direction back when wall sliding ends.
            if (_movement.CurrentState != CharacterStates.MovementStates.WallClinging) 
            {
